function MatTracking = ULM_localization2D(MatIn, ULM)
  ...
  %           - LocMethod : localisation mehtod {'wa','interp-cubic','interp-lanczos3','interp-triangle','radial','curvefitting','nolocalization'};
  %           - InterpMethod : {'bicubic','lanczos3','spline'} (methods available for imresize)
  ...
  for iscat = 1:size(index_mask_z, 1)
    ...
    switch lower(ULM.LocMethod)
      case 'radial'
        [Zc, Xc, ~] = LocRadialSym(IntensityRoi, fwhmz, fwhmx);
      case 'wa'
        [Zc, Xc, ~] = LocWeightedAverage(IntensityRoi, vectfwhmz, vectfwhmx);
      case 'interp-cubic'
        [Zc, Xc, ~] = LocInterp(IntensityRoi, 'cubic', vectfwhmz, vectfwhmx);
      case 'interp-triangle'
        [Zc, Xc, ~] = LocInterp(IntensityRoi, 'triangle', vectfwhmz, vectfwhmx);
      case 'interp-lanczos3'
        [Zc, Xc, ~] = LocInterp(IntensityRoi, 'lanczos3', vectfwhmz, vectfwhmx);
      case 'curvefitting'
